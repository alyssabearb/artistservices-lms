/**
 * Page 4 – Block A: Section Content ONLY (no resource cards).
 * Data source: Training Sections.
 * Use with Block B (ResourcesOnly) + Block C (Resource Library) for custom resource card formatting.
 *
 * Setup: Section Detail page should have THREE blocks in this order:
 * 1. This block (Section Content) – data: Training Sections
 * 2. ResourcesOnly block – data: Training Sections (syncs resourceIds to URL)
 * 3. Resource Library block – data: Resource Library (reads resourceIds from URL, renders cards + nav)
 *
 * This block renders: Back to All Pages, Section Title (red), body/video/checklist/survey/gallery only.
 * Prev/Next appear here ONLY when the section has NO linked resources.
 * When the section HAS linked resources, Prev/Next appear in the Resource Library block below the cards.
 */
...
function getParamsFromUrl() {
  if (typeof window === "undefined") return { recordId: null, courseId: null, trackId: null, sectionIds: [], personId: null };
...
export default function Block() {
  ...
  else if (sectionTypeLower === "survey" || sectionTypeLower === "submission" || sectionTypeLower.indexOf("survey") !== -1 || sectionTypeLower.indexOf("submission") !== -1) sectionType = "Survey";
  ...
  var linkedResRaw = get("linkedResources", "Linked Resources") || getByPartial("linked") || getByPartial("resource");
  var linkedArray = Array.isArray(linkedResRaw) ? linkedResRaw : [];
  var hasLinkedResources = linkedArray.length > 0;
  ...
  var showNav = sectionIds.length >= 1 && !hasLinkedResources;
  ...
  if (sectionType === "Survey") {
    ...
    if (surveySrc) {
      ...
      contentEls.push(
        React.createElement("div", { key: "survEmbed", className: "w-full" },
          React.createElement("div", { className: "w-full min-h-[600px] rounded-lg overflow-hidden border border-border" }, React.createElement("iframe", { src: surveyHref, title: "Survey", className: "w-full min-h-[600px] border-0", style: { minHeight: "600px" } })),
          React.createElement("p", { className: "mt-3 text-sm text-muted-foreground" }, "If the form doesn\u2019t appear above, ", React.createElement("a", { href: surveyHref, target: "_blank", rel: "noopener noreferrer", className: "text-primary underline" }, "open it in a new tab"), ".")
        )
      );
  ...
  return React.createElement("div", { className: "w-full max-w-full px-4 md:px-6 lg:px-8 py-6 md:py-8" },
